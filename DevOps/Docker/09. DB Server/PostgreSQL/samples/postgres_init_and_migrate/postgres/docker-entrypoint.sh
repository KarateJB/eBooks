...
pg_create_pgpass() {
	...
	echo localhost:5433:*:$POSTGRES_USER:$POSTGRES_PASSWORD > /var/lib/postgresql/data/.pgpass;
...
_main() {
	...
	if [ "$1" = 'postgres' ] && ! _pg_want_help "$@"; then
		...
		if [ -z "$DATABASE_ALREADY_EXISTS" ]; then
            ...
            # Disable running docker_process_init_files
        ...
        # Create pgpass file
        echo "$dt [Start] Create pgpass..."
        pg_create_pgpass
        echo "$dt [End] Create pgpass."

        # Execute PSQLs
        echo "$dt [Start] Run PSQLs.sql..."
        sh /usr/local/bin/run_psql.sh
        echo "$dt [End] Run PSQLs.sql."

        # DB migration
        echo "$dt [Start] DB migration..."
        sh /usr/local/bin/migrate_db.sh
        echo "$dt [End] DB migration."

        # Run services
		echo "$dt [Start] Run services..."
        sh /usr/local/bin/run_services.sh
		echo "$dt [End] Run services."
